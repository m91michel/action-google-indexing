...
export const run = async () => {
  try {
    ...
    core.info(``)
    core.info(`👍 Done, here's the status of all processed pages:`)
    mkdirSync('.cache', { recursive: true })
    writeFileSync(cachePath, JSON.stringify(statusPerUrl, null, 2))

    const processedPagesCount = Object.keys(statusPerUrl).length;
    core.info(`📊 Processed ${processedPagesCount} out of ${pages.length} pages`)
